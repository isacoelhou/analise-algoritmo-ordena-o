...
func merge(left, right []Person, stats *SortStats) []Person {
	...
	for i < len(left) && j < len(right) {
		if left[i].Number < right[j].Number {
			...
			// Números são iguais, comparar pelos nomes
			if strings.Compare(left[i].FirstName, right[j].FirstName) < 0 {
				result = append(result, left[i])
				i++
			} else if strings.Compare(left[i].FirstName, right[j].FirstName) > 0 {
				result = append(result, right[j])
				j++
			} else {
				// Nomes são iguais, comparar pelos sobrenomes
				if strings.Compare(left[i].LastName, right[j].LastName) <= 0 {
					result = append(result, left[i])
					i++
				} else {
					result = append(result, right[j])
...
func insertionSort(items []Person, stats *SortStats) {
	...
	for i := 1; i < n; i++ {
		j := i
		for j > 0 && comparePeople(items[j-1], items[j]) > 0 {
...
func comparePeople(a, b Person) int {
	...
	if cmp := strings.Compare(a.FirstName, b.FirstName); cmp != 0 {
		return cmp
	}
	if cmp := strings.Compare(a.LastName, b.LastName); cmp != 0 {
		return cmp
	}
...
func main() {
	...
	out, ferror1 := os.Create("pessoas_ordenados.txt")
	...
	for end <= len(people) {
		...
		// Usar o insertion sort para ordenar o subconjunto
